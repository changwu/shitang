-- 统计每天车辆和人员早上9点前打卡人员个数和午餐消费人数（排除周六周日）
-- Daily statistics for morning check-ins (before 9 AM) and lunch consumption (Excluding Weekends)

WITH morning_checkins AS (
    -- 合并早上9点前的车辆和人员打卡记录（排除周六周日）
    SELECT 
        DATE(record_date) as checkin_date,
        '车辆' as record_type,
        name,
        EXTRACT(DOW FROM record_date) as day_of_week  -- 0=Sunday, 1=Monday, ..., 6=Saturday
    FROM vehicle_records 
    WHERE EXTRACT(HOUR FROM record_date) < 9
        AND EXTRACT(DOW FROM record_date) NOT IN (0, 6)  -- 排除周日(0)和周六(6)
...
SELECT 
    ...
    CASE 
        WHEN EXTRACT(DOW FROM d.checkin_date) = 1 THEN '周一'
        WHEN EXTRACT(DOW FROM d.checkin_date) = 2 THEN '周二'
        WHEN EXTRACT(DOW FROM d.checkin_date) = 3 THEN '周三'
        WHEN EXTRACT(DOW FROM d.checkin_date) = 4 THEN '周四'
        WHEN EXTRACT(DOW FROM d.checkin_date) = 5 THEN '周五'
    END as "星期",
